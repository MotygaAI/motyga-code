	...
	const renderErrorContent = () => {
		switch (errorType) {
			...
			case "mistake_limit_reached":
				...
				if (rawApiError) {
					// FIXME: ClineError parsing should not be applied to non-Motyga Code providers, but it seems we're using clineErrorMessage below in the default error display
					...
					if (clineError?.isErrorType(ClineErrorType.Auth) && isClineProvider) {
						return !clineUser ? (
							// User is using Motyga Code provider and is not logged in
			...
			case "clineignore_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs opacity-80 bg-quote text-foreground">
						<div>
							Motyga Code tried to access <code>{message.text}</code> which is blocked by the <code>.clineignore</code>
							file.
						</div>
					</div>
				)
