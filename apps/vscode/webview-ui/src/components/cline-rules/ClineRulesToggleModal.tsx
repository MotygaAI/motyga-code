...
const ClineRulesToggleModal: React.FC = () => {
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleClineRule(
			...
			.catch((error) => {
				console.error("Error toggling Motyga Code rule:", error)
			})
	}
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Motyga Code Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Motyga Code Rules & Workflows" : "Show Motyga Code Rules & Workflows"}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Motyga Code with system-level guidance. Think of them as a persistent way
								...
								<p>
									Workflows allow you to define a series of steps to guide Motyga Code through a repetitive set of
								...
								<p>
									Skills are reusable instruction sets that Motyga Code can activate on-demand. When a task matches a
									skill's description, Motyga Code uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Motyga Code's execution lifecycle,
